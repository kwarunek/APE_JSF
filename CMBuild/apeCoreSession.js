...
Native.genericize = function(object, property, check) {
    if ((!check || !object[property])
            && typeof object.prototype[property] == 'function')
...
function $each(iterable, fn, bind) {
    ...
    ((type == 'arguments' || type == 'collection' || type == 'array') ? Array
            : Hash).each(iterable, fn, bind);
}
;
...
function $mixin(mix) {
    for (var i = 1, l = arguments.length; i < l; i++) {
        ...
        for (var key in object) {
            ...
            mix[key] = (mp && $type(op) == 'object' && $type(mp) == 'object') ? $mixin(
                    mp, op)
                    : $unlink(op);
        }
    }
...
function $type(obj) {
    if (obj == undefined)
        return false;
    if (obj.$family)
        return (obj.$family.name == 'number' && !isFinite(obj)) ? false
                : obj.$family.name;
    if (obj.nodeName) {
        switch (obj.nodeType) {
            case 1:
 ...
 * script: Browser.js
 * 
 * description: The Browser Core. Contains Browser initialization, Window and
...
function $exec(text) {
    ...
    if (window.execScript) {
        ...
        script[(Browser.Engine.webkit && Browser.Engine.version < 420) ? 'innerText'
                : 'text'] = text;
            ...
            initialize: function(win) {
                ...
                if (!win.Element) {
                    ...
                    if (Browser.Engine.webkit)
                        win.document.createElement("iframe"); // fixes safari
                    // 2
    ...
    flatten: function() {
        ...
        for (var i = 0, l = this.length; i < l; i++) {
            ...
            if (!type)
                continue;
            array = array
                    .concat((type == 'array' || type == 'collection' || type == 'arguments') ? Array
                    .flatten(this[i])
                    : this[i]);
        }
        return array;
 ...
 * script: Hash.js
 * 
 * description: Contains Hash Prototypes. Provides a means for overcoming the
 * JavaScript practical impossibility of extending native Objects.
 * 
 * license: MIT-style license.
 * 
 * requires: - /Hash.base
...
var Event = new Native(
        {
            ...
            initialize: function(event, win) {
                ...
                if (type.test(/key/)) {
                    ...
                    var client = {
                        x: (event.pageX) ? event.pageX - win.pageXOffset
                                : event.clientX,
                        y: (event.pageY) ? event.pageY - win.pageYOffset
                                : event.clientY
                    };
...
var Cookie = new Class({
    ...
    read: function() {
        // [PK]
        var checkValue = this.options.document.cookie.match(new RegExp(this.key
                .escapeRegExp(), 'g'));
...
var Request = new Class(
        {
            ...
            options: {/*
             * onRequest: $empty, onComplete: $empty, onCancel:
             * $empty, onSuccess: $empty, onFailure: $empty,
             * onException: $empty,
             */
                url: '',
                data: '',
...
APEDBL.Events = new Class({
    ...
    removeEvent: function(type, fn) {
        return Events.prototype.removeEvent.run([type,
            this.$originalEvents[type][fn]], this);
    }
...
APEDBL.Core = new Class(
        {
            ...
            options: {
                ...
                transport: 0, // Transport 0: long polling, 1 : XHRStreaming,
                // 2: JSONP, 3 SSE / JSONP, 4 : SSE / XHR
                frequency: 0, // Frequency identifier
                cycledStackTime: 350, // Time before send request of
                // cycledStack
                secure: false
            },
            initialize: function(options) {
            ...
            selectTransport: function() {
                ...
                while (support !== true) {
                    support = transports[transport].browserSupport();// Test
                    // if
                    // browser
                    // support
                    // transport
                    ...
                    if (support === true) {
                        ...
                        transport = support;// Browser do not support transport,
                    // next loop will test with fallback
                    // transport returned by
                    // browserSupport();
                }
            },
            poller: function() {
            ...
            stopRequest: function() {
                this.cancelRequest();
                if (this.transport.streamRequest)
                    this.transport.streamRequest.cancel();
                this.requestDisabled = true;
            },
            parseParam: function(param) {
                return ($type(param) == 'object') ? Hash.getValues(param)
                        : $splat(param);
            },
                    ...
                    for (var i = 0; i < raws.length; i++) { // Read all raw
                        ...
                        if (!this.transport.running()) {
                            if (!raw.data.code
                                    ...
                                    && raw.data.code != '005'
                                    && raw.data.code != '001'
                                    && raw.data.code != '004' && raw.data.code != '003')) {
                                check = true;
                            }
                        } else {
                            // Invalidate check if something went wrong with
                            // other raw or a new request have been launched
                            check = false;
                        }
                    }
                } else if (!this.transport.running())
                    check = true; // No request running, request didn't
                // respond correct JSON, something went
                // wrong
                if (check)
                    this.check();
            },
            /*******************************************************************
             * Fire raw event. If received raw is on a non-existing pipe, create
             * new pipe
             */
            callRaw: function(raw) {
                ...
                if (raw.data.chl) {// Execute callback on challenge
                    var chlCallback = this.request.callbackChl
                            .get(raw.data.chl);
                    if (chlCallback) {
            ...
            getRequest: function(opt) {
                if (!opt.request)
                    ...
                    return this.request[opt.request].add
                            .bind(this.request[opt.request]);
            },
            /*******************************************************************
            ...
            left: function(pubid) {
                this.request.send('LEFT', {
                    "channel": this.pipes.get(pubid).name
                });
            },
            quit: function() {
...
APEDBL.Pipe = new Class({
    ...
    initRequestMethod: function() {
        ...
        this.request = {
            ...
            cycledStack: {
                add: function() {
                    var args = this.parsePipeCmd.apply(this, arguments);
                    this.ape.request.cycledStack.add.apply(
                            this.ape.request.cycledStack, args);
                }.bind(this),
                send: this.ape.request.send,
                setTime: this.ape.request.cycledStack.setTime
                        .bind(this.ape.request.cycledStack)
            },
            stack: {
                add: function() {
                    var args = this.parsePipeCmd.apply(this, arguments);
                    this.ape.request.stack.add.apply(this.ape.request.stack,
                            args);
                }.bind(this),
    ...
    parsePipeCmd: function() {
        // convert arguments to a real array to avoid a bug with firefox see bug
        // #292215 https://bugzilla.mozilla.org/show_bug.cgi?id=292215
        var args = Array.prototype.slice.call(arguments);
...
APEDBL.PipeProxy = new Class({
    ...
    open: function(hostname, port) {
        if (this.core.status == 0)
            this.core.start(null, false);
        // Adding a callback to request response to create a new pipe if
        // this.pipe haven't been init
        this.request.stack.add('PROXY_CONNECT', {
            'host': hostname,
            'port': port
        }, this.pipe ? {} : {
...
APEDBL.PipeMulti = new Class({
    ...
    rawLeft: function(raw, pipe) {
        ...
        if (raw.data.user.pubid == this.ape.user.pubid)
            this.ape.delPipe(pipe.pipe.pubid);
    },
    left: function() {
    ...
    getUserPipe: function(user) {
        if (typeof user == 'string')
            user = this.users.get(users.pubid);
        return this.ape.newPipe('uni', {
            'pipe': user
        });
    }
});
...
APEDBL.Request = new Class(
        {
            ...
            send: function(cmd, params, options, noWatch) {
                ...
                var ret = this.ape.transport.send(this.parseCmd(cmd, params,
                        opt), opt);

                $clear(this.ape.pollerObserver);
                this.ape.pollerObserver = this.ape.poller.delay(
                        this.ape.options.pollTime, this.ape);
            ...
            parseCmd: function(cmd, params, options) {
                ...
                if ($type(cmd) == 'array') {
                    ...
                    if (!$defined(options.sessid) || options.sessid !== false)
                        o.sessid = this.ape.sessid;
                    a.push(o);
            ...
            /*******************************************************************
             * This method is only used by opera. Opera have a bug, when request
             * are sent trought user action (ex : a click), opera throw a
             * security violation when trying to make a XHR. The only way is to
             * set a class var and watch when this var change
             */
            requestObserver: function() {
                if (this.requestVar.updated) {
                    var args = this.requestVar.args.shift();
                    this.requestVar.updated = (this.requestVar.args.length > 0) ? true
                            : false;
...
APEDBL.Request.CycledStack = new Class({
    initialize: function(ape) {
        ...
        this.timer = this.send.periodical(this.ape.options.cycledStackTime,
                this);
    },
...
Request = new Class({
    Extends: Request,
    send: function(options) {
        // mootools set onreadystatechange after xhr.open, in webkit, this cause
        // readyState 1 to be never fired
        if (Browser.Engine.webkit)
            this.xhr.onreadystatechange = this.onStateChange.bind(this);
...
APEDBL.Transport.longPolling = new Class({
    ...
    send: function(queryString, options) {
        ...
        this.requestFailObserver.push(this.ape.requestFail.delay(
                this.ape.options.pollTime + 10000, this.ape, [-1, request]));
...
Request.XHRStreaming = new Class({
    ...
    onStateChange: function() {
        if (this.xhr.readyState == 1)
            ...
            this.progress(this.xhr.responseText, this.xhr.responseXML);
        this.parent();
    },
    onProgress: function() {
...
APEDBL.Transport.XHRStreaming = new Class(
        {
            ...
            initialize: function(ape) {
                ...
                // If browser support servent sent event, switch to SSE / XHR
                // transport
                if (this.SSESupport)
                    this.ape.options.transport = 4;
            ...
            send: function(queryString, options) {
                if (this.SSESupport && !this.eventSource) {
                    ...
                    if (options.requestCallback)
                        this.streamInfo.callback = options.requestCallback;
                } else {
                    if ((!this.streamRequest || !this.streamRequest.running)
                            && !this.eventSource) { // Only one XHRstreaming
                        // request is allowed
                        this.buffer = '';
                        ...
                        var request = new Request({
                            url: this.ape.serverUri,
                            onFailure: this.ape.requestFail.bind(this.ape, [
                                -2, this]),
                            onComplete: function(resp) {
                                $clear(this.requestFailObserver.shift());
                                this.request.dataSent = true;// In the case
                                // of
                                // XHRStreaming.
                                // Request are
                                // imediatly
                        ...
                        this.requestFailObserver.push(this.ape.requestFail
                                .delay(this.ape.options.pollTime + 10000,
                                this.ape, [1, request]));
            ...
            readFragment: function(text) {
                ...
                if (text == '') {
                    ...
                    // If group.length is gretter than 1 the fragment received
                    // complete last RAW or contains more than one RAW
                    if (group.length > 1) {
                        // Clear buffer
                        this.buffer = '';

                        for (var i = 0; i < length - 1; i++) {
                            this.ape.parseResponse(group[i],
                                    this.streamInfo.callback);
                        }

                        if (group[length - 1] !== '') { // Last group complete
                            // last received raw but
                            // it's not finish
            ...
            running: function() {
                return (this.streamRequest && this.streamRequest.running) ? true
                        : this.eventSource ? true : false;
            },
            checkStream: function() {
                return (this.streamInfo.forceClose && this.streamInfo.canClose)
                        || (this.streamRequest
                        && this.streamRequest.read >= this.maxRequestSize && this.streamInfo.canClose);
            },
            newStream: function() {
                // this.ape.request.send('CLOSE');//This will close the stream
                // request
                $clear(this.streamInfo.timeoutObserver);
...
APEDBL.Transport.XHRStreaming.browserSupport = function() {
    if (Browser.Features.xhr && (Browser.Engine.webkit || Browser.Engine.gecko)) {
        return true;
        /*
         * Not yet if (Browser.Engine.presto && ((typeof window.addEventStream) ==
         * 'function')) return true; else if (window.XDomainRequest) return
         * true; else return Browser.Engine.trident ? 0 : true;
         */
    } else if (Browser.Features.xhr)
        return 0;// No XHRStreaming support switch to long polling
    else
...
APEDBL.Transport.JSONP = new Class({
    ...
    running: function() {
        /*
         * if (this.SSESupport) { return this.eventSource ? true : false; } else {
         */
...
APEDBL.Transport.WebSocket = new Class({
    ...
    initWs: function() {
        this.ws = new WebSocket((this.ape.options.secure ? 'wss' : 'ws')
                + '://' + this.ape.options.frequency + '.'
                + this.ape.options.server + '/' + this.ape.options.transport
                + '/');
    ...
    openWs: function() {
        if (this.stack.length > 0) {
            for (var i = 0; i < this.stack.length; i++)
                this.send(this.stack[i].q, this.stack[i].options);
            this.stack.length = 0;
        }
    },
    closeWs: function() {
...
APEDBL.Transport.WebSocket.browserSupport = function() {
    if ('WebSocket' in window)
        ...
        return 1;// No websocket support switch to XHRStreaming
}
String.implement({
    addSlashes: function() {
...
var B64 = new Hash({
    ...
    /***************************************************************************
     * Base 64 encode / Base 64 decode Taken from orbited project -
    ...
    decode: function(str) {
        ...
        for (var i = 0; i < l; ) {
            ...
            if (i <= l)
                t |= B64.$tab.indexOf(s[i++]) << 12;
            if (i <= l)
                t |= B64.$tab.indexOf(s[i++]) << 6;
            if (i <= l)
                t |= B64.$tab.indexOf(s[i++]);
...
try {
    // Avoid showing error if window.parent.setInterval() is not working (ie :
    // permission denied)
    // console.log('setInterval', fn, time);
    window.parent.setInterval();

    // Override setInterval to be done outside the frame (there is some issue
    // inside the frame with FF3 and WebKit)
    if (!Browser.Engine.trident && !Browser.Engine.presto
            && !(Browser.Engine.gecko && Browser.Engine.version <= 18)) {
...
(function() {
    ...
    var cx = /[\u0000\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, escapable = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, gap, indent, rep;
    ...
    if (typeof JSON.stringify !== 'function' || (navigator.product == 'Gecko' &&
            // There is a know bug with some version of FF 3.5 with the replacer
                    // parameters, this test, is to check if the browser havent a bugy version
                            // of JSON.stringify
                                    (function() {
...
APEDBL.Core = new Class(
        {
            ...
            saveSessionPipe: function() {
                ...
                this.setSession({
                    'uniPipe': JSON.stringify(uniPipe)
                });
            },
            restoreUniPipe: function(resp) {
                var pipes = JSON
                        .parse(decodeURIComponent(resp.data.sessions.uniPipe));
                if (pipes) {
             ...
             * 
             * @param String
             *                identifier, can be used to force the identifier to
             *                find ortherwhise identifier defined in the options
             *                will be used
             * @return Boolean false if application identifier isn't found or an
            ...
            getInstance: function(identifier) {
                ...
                for (var i = 0; i < tmp.instance.length; i++) {
                    if (tmp.instance[i]
                            && tmp.instance[i].identifier == identifier) {
                        return {
                            instance: tmp.instance[i],
                            cookie: tmp
                        };
                    }
                }
            ...
            /*******************************************************************
             * Initialize cookie and some application variable is instance is
             * found set this.cookie variable
             * 
             * @return boolean true if instance is found, else false
             */
            initCookie: function() {
                var tmp = this.getInstance();
                if (tmp && tmp.instance) { // Cookie exist, application
                    // instance exist
                    ...
                    return true;
                } else if (tmp.cookie) { // Cookie exist, no application
                    // instance
            ...
            /*******************************************************************
             * Create a cookie instance (add to the instance array of the cookie
            ...
            createCookie: function() {
                if (!this.cookie) {
                    // No Cookie or no ape instance in cookie, lets create the
                    // cookie
                    var tmp = {
                        frequency: 1,
                        instance: []
